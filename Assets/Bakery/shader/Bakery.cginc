...
#if defined(MLS_BLENDING_STANDARD)
#include "MLS_Standard_Common.cginc"
#endif
...
#ifdef BAKERY_BICUBIC
    // Bicubic
#if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
    float4 BakeryTex2D(sampler2D tex, float2 uv, float4 texelSize, int textId)
    {
        ...
        return  BakeryBicubic_g0(fy) * (g0x * BlendBakeryRNM(textId, (float2(px + h0x, py + h0y) * texelSize.x)) +
                    g1x * BlendBakeryRNM(textId, (float2(px + h1x, py + h0y) * texelSize.x))) +
                BakeryBicubic_g1(fy) * (g0x * BlendBakeryRNM(textId, (float2(px + h0x, py + h1y) * texelSize.x)) +
                    g1x * BlendBakeryRNM(textId, (float2(px + h1x, py + h1y) * texelSize.x)));
    }
#endif
...
#else
    // Bilinear
    float4 BakeryTex2D(sampler2D tex, float2 uv, float4 texelSize, int textId = 0)
    {
        #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
        {
            return lerp(tex2D(tex, uv), BlendBakeryRNM(textId, uv), _MLS_ENABLE_LIGHTMAPS_BLENDING);
        }
        #else
        {
            return tex2D(tex, uv);
...
#if UNITY_VERSION >= 201740
        #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
            float3 dominantDir = lerp(unity_LightmapInd.Sample(bakery_trilinear_clamp_sampler, lmUV).xyz * 2 - 1, BlendTwoTextures(1, lmUV).xyz * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
        #else
            float3 dominantDir = unity_LightmapInd.Sample(bakery_trilinear_clamp_sampler, lmUV).xyz * 2 - 1;
        #endif    
#else
        #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
            float3 dominantDir = lerp(UNITY_SAMPLE_TEX2D_SAMPLER(unity_LightmapInd, unity_Lightmap, lmUV).xyz * 2 - 1, BlendTwoTextures(1, lmUV).xyz * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
        #else
...
void BakeryRNM(inout float3 diffuseColor, inout float3 specularColor, float2 lmUV, float3 normalMap, float smoothness, float3 viewDirT)
{
    ...
    #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
        float3 rnm0 = lerp(DecodeLightmap(BakeryTex2D(_RNM0, lmUV, _RNM0_TexelSize)), DecodeLightmap(BakeryTex2D(_RNM0, lmUV, _RNM0_TexelSize, 0)), _MLS_ENABLE_LIGHTMAPS_BLENDING);
        float3 rnm1 = lerp(DecodeLightmap(BakeryTex2D(_RNM1, lmUV, _RNM0_TexelSize)), DecodeLightmap(BakeryTex2D(_RNM1, lmUV, _RNM0_TexelSize, 1)), _MLS_ENABLE_LIGHTMAPS_BLENDING);
        float3 rnm2 = lerp(DecodeLightmap(BakeryTex2D(_RNM2, lmUV, _RNM0_TexelSize)), DecodeLightmap(BakeryTex2D(_RNM2, lmUV, _RNM0_TexelSize, 2)), _MLS_ENABLE_LIGHTMAPS_BLENDING);
    #else
...
void BakerySH(inout float3 diffuseColor, inout float3 specularColor, float2 lmUV, float3 normalWorld, float3 viewDir, float smoothness)
{
    ...
    #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
        float3 nL1x = lerp(BakeryTex2D(_RNM0, lmUV, _RNM0_TexelSize) * 2 - 1, BakeryTex2D(_RNM0, lmUV, _RNM0_TexelSize, 0) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
        float3 nL1y = lerp(BakeryTex2D(_RNM1, lmUV, _RNM0_TexelSize) * 2 - 1, BakeryTex2D(_RNM1, lmUV, _RNM0_TexelSize, 1) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
        float3 nL1z = lerp(BakeryTex2D(_RNM2, lmUV, _RNM0_TexelSize) * 2 - 1, BakeryTex2D(_RNM2, lmUV, _RNM0_TexelSize, 2) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
    #else
...
half4 bakeryFragForwardBase(BakeryVertexOutputForwardBase i) : SV_Target
{
...
#ifdef BAKERY_VOLUME
    #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
        mainLight.color *= saturate(dot(lerp(_VolumeMask.Sample(sampler_Volume0, lpUV), BlendBakeryVolume(3, lpUV), _MLS_ENABLE_LIGHTMAPS_BLENDING), unity_OcclusionMaskSelector));
    #else
    ...
    UnityGI gi = FragmentGI(s, occlusion, i.ambientOrLightmapUV, atten, mainLight);
        ...
        #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
            tex0 = lerp(_Volume0.Sample(sampler_Volume0, lpUV), BlendBakeryVolume(0, lpUV), _MLS_ENABLE_LIGHTMAPS_BLENDING);
            tex1 = lerp(_Volume1.Sample(sampler_Volume0, lpUV) * 2 - 1, BlendBakeryVolume(1, lpUV) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
            tex2 = lerp(_Volume2.Sample(sampler_Volume0, lpUV) * 2 - 1, BlendBakeryVolume(2, lpUV) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
            tex3 = lerp(_Volume3.Sample(sampler_Volume0, lpUV) * 2 - 1, BlendBakeryVolume(4, lpUV) * 2 - 1, _MLS_ENABLE_LIGHTMAPS_BLENDING);
        #else
        ...
        #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
            tex0 = lerp(_Volume0.Sample(sampler_Volume0, lpUV), BlendBakeryVolume(0, lpUV), _MLS_ENABLE_LIGHTMAPS_BLENDING);
            tex1 = lerp(_Volume1.Sample(sampler_Volume0, lpUV), BlendBakeryVolume(1, lpUV), _MLS_ENABLE_LIGHTMAPS_BLENDING);
            tex2 = lerp(_Volume2.Sample(sampler_Volume0, lpUV), BlendBakeryVolume(2, lpUV), _MLS_ENABLE_LIGHTMAPS_BLENDING);
...
half4 bakeryFragForwardAdd(BakeryVertexOutputForwardAdd i) : SV_Target
{
    ...
    #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
        c *= saturate(dot(lerp(_VolumeMask.Sample(sampler_Volume0, lpUV), BlendBakeryVolume(3, lpUV), _MLS_ENABLE_LIGHTMAPS_BLENDING), unity_OcclusionMaskSelector));
    #else
...
// Baked direct lighting occlusion if any
    ...
    #ifdef BAKERY_VOLUME
        #if defined(MLS_BLENDING_STANDARD) || defined(MLS_BLENDING_SRP)
            outShadowMask = lerp(_VolumeMask.Sample(sampler_Volume0, lpUV), BlendBakeryVolume(3, lpUV), _MLS_ENABLE_LIGHTMAPS_BLENDING);
        #else
