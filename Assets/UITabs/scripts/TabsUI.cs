...
namespace EasyUI.Tabs {
   ...
   public abstract class TabsUI : MonoBehaviour {
      ...
      [SerializeField] private GameObject myTabContainer; //QUESTO
      [SerializeField] private GameObject myHelpContainer; //Test

      private Transform parentHelp; // Aggiunto
      private GameObject[] tabHelp; // Aggiunto
      ...
      private void GetTabBtns () {
         ...
         parentContent = myTabContainer.transform; //QUESTO
         tabBtnsNum = parentBtns.childCount;
    tabContentNum = parentContent.childCount;
         if(myHelpContainer != null){
            parentHelp = myHelpContainer.transform;
            tabHelp = new GameObject[tabBtnsNum];
            } //Test
         ...
         for (int i = 0; i < tabBtnsNum; i++) {
            ...
            if(myHelpContainer != null){
               tabHelp [i] = parentHelp.GetChild(i).gameObject;//Test
               //tabHelp [i].SetActive(false); //Test
               }
      ...
      public void OnTabButtonClicked (int tabIndex) {
         if (current != tabIndex) {
            ...
            if(myHelpContainer != null){
             tabHelp [previous].SetActive(false); //Test
             tabHelp [current].SetActive(true); //Test
             }
