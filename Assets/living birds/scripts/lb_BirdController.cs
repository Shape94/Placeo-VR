...
public class lb_BirdController : MonoBehaviour {
	...
	void SpawnBird(){
		if (!pause){
			...
			//bird.transform.position = new Vector3(4,4,-2);
	...
	Vector3 FindPositionOffCamera(){
		...
		Vector3 spawnPosition = currentCamera.transform.position + (ray * dist);
		spawnPosition.y += 6f; // Aggiunge 3 metri all'asse y
